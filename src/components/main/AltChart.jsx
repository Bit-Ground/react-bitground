...
export default function AltChart() {
    ...
    const lastToday = todayData.at(-1)?.altIndex ?? 0;
    const todayColor = lastToday >= yesterdayAvg ? '#FC5754' : '#2979ff';

    return (
        <AltCard
            title="BGAI"
            subtitle="BitGround Alt Index"
            value={lastToday}
            diffValue={lastToday - (yesterdayData.at(-1)?.altIndex ?? 0)}
